...
<p align="center">
  <strong>Even the official MCP reference server scores <code>44/100</code>.</strong><br>
  Most MCP servers just aren't built for <em>agents</em> to use well&nbsp;— yet.<br>
  <code>mcp-scorecard</code> boots any server over stdio and shows exactly where yours stands, and how to fix it.<br>
  <sub>One command · no target credentials · <a href="https://github.com/davidmosiah/mcp-leaderboard">see the full leaderboard&nbsp;→</a></sub>
</p>
...
`MCP_PROBE=1` set on a minimal child env. It does **not** forward your shell
tokens or API keys to the target. **Author hook:** if your MCP needs OAuth or
other credentials to even list tools, detect this env var and return your
tool/resource/prompt manifests anyway. The scorecard expects to be able to
read your contract without making any auth-requiring API calls.
...
- Any string going into the report is run through a redaction pass that
  ...
  `[REDACTED]`.
- No telemetry from `mcp-scorecard`.
- Auditing an npm package or GitHub repo executes that target's MCP server
  code. The scorecard does not pass credentials, but this is not a sandbox:
  use an isolated runner/container for untrusted packages.
